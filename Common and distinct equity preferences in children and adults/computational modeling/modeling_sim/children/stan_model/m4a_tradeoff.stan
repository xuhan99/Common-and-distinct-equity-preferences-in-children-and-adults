...
parameters {
  ...
  vector[9] mu_pr; //mean of the parameters,4 advantageous inequality aversion, 4 disadvantageous inequality aversion and a inverse temperature.
  vector<lower=0>[9] sigma; //variance of the parameters, 2 paras
  
  // Subject-level raw parameters (for Matt trick)
  vector[Ns] alpha_pr11;  // alpha: advantageous inequality aversion for FP_pos
  vector[Ns] alpha_pr12;  // alpha: advantageous inequality aversion for FP_neg
  vector[Ns] alpha_pr21;  // alpha: advantageous inequality aversion for TP_pos
  vector[Ns] alpha_pr22;  // alpha: advantageous inequality aversion for TP_neg
...
transformed parameters {
  ...
  for (i in 1:Ns) {
    ...
    beta11[i]  = mu_pr[5] + sigma[5] * beta_pr11[i];
    beta12[i]  = mu_pr[6] + sigma[6] * beta_pr12[i];
    beta21[i]  = mu_pr[7] + sigma[7] * beta_pr21[i];
    beta22[i]  = mu_pr[8] + sigma[8] * beta_pr22[i];
    tau[i]    = Phi_approx(mu_pr[9] + sigma[9] * tau_pr[i]) * 20;
  }
}
...
model {
  ...
  for (i in 1:Ns) {
    ...
    util[cn] = Xself - alpha_temp*fmax(Xself-Xother,0) - beta_temp*fmax(Xother-Xself,0); // utility of each possible choices
    }
